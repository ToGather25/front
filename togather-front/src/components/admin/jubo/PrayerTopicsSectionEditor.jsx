...
import { getPrayerTopics, updateJuboSection } from "@/services/juboService";
...
export default function PrayerTopicsSectionEditor({ churchId, juboId }) {
  const {
    data: initial,
    loading: prefillLoading,
    error: prefillError,
    refetch: refetchPrefill,
  } = useFetch(() => getPrayerTopics(churchId), [churchId], null);
  ...
  function addRow() {
    setRows((prev) => [...prev, { title: "", subtitle: "", category: "" }]);
  }
  ...
  async function handleSave() {
    ...
    setSaveError(false);
    try {
      await updateJuboSection(churchId, juboId, "PRAYER_TOPICS", rows);
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
    } catch (err) {
      console.error("[PrayerTopicsSectionEditor] 저장 실패:", err);
      setSaveError(true);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-body-3 font-bold text-grey-10">기도제목</h3>
