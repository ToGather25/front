...
import { getNews, updateJuboSection } from "@/services/juboService";
...
export default function NewsSectionEditor({ churchId, juboId }) {
  const {
    data: initial,
    loading: prefillLoading,
    error: prefillError,
    refetch: refetchPrefill,
  } = useFetch(() => getNews(churchId), [churchId], null);
  ...
  async function handleSave() {
    ...
    setSaveError(false);
    try {
      await updateJuboSection(churchId, juboId, "NEWS", groups);
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
    } catch (err) {
      console.error("[NewsSectionEditor] 저장 실패:", err);
      setSaveError(true);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-body-3 font-bold text-grey-10">교회 소식</h3>
