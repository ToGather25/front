...
function TrendIcon({ trend }) {
  if (trend > 0)
    return <span className="text-[10px] text-blue-7 font-bold">▲{Math.abs(trend)}</span>;
  if (trend < 0)
    return <span className="text-[10px] text-red-400 font-bold">▼{Math.abs(trend)}</span>;
        ...
        {rows.length === 0 ? (
          ...
          <div className="overflow-y-auto max-h-[60vh]">
            {rows.map(({ rank, name, count, trend }, i) => (
              <div
                key={rank}
                className={`flex items-center px-4 py-3 gap-3 ${i < rows.length - 1 ? "border-b border-bluegrey-2" : ""}`}
              >
                <span className="w-5 text-body-4 text-grey-7 font-medium shrink-0 text-center">
                  {rank}
                </span>
                <span className="flex-1 text-body-3 text-grey-10 truncate">{name}</span>
                <span className="text-body-4 text-grey-6 shrink-0">
                  {formatKorean(count)}
                  {unit}
                </span>
                {trend !== undefined && (
      ...
      <div className="flex items-end justify-center gap-8 mb-10">
        {isNewUser ? (
          <div className="flex flex-col items-center gap-3 py-8">
            ...
            <p className="text-body-3 font-semibold text-grey-8">
              첫 구절을 읽고 랭킹에 진입해보세요!
            </p>
