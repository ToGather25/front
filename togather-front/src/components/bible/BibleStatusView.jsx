...
// 성경 권별 절 수 (개역개정 기준)
const VERSE_COUNT = {
  창:1533,출:1213,레:859,민:1288,신:959,수:658,삿:618,룻:85,
  삼상:810,삼하:695,왕상:816,왕하:719,대상:942,대하:822,스:280,느:406,에:167,
  욥:1070,시:2461,잠:915,전:222,아:117,사:1292,렘:1364,애:154,
  겔:1273,단:357,호:197,욜:73,암:146,옵:21,욘:48,미:105,나:47,합:56,습:53,학:38,슥:211,말:55,
  마:1071,막:678,눅:1151,요:879,행:1007,롬:433,고전:437,고후:257,갈:149,엡:155,
  빌:104,골:95,살전:89,살후:47,딤전:113,딤후:83,딛:46,몬:25,히:303,
  약:108,벧전:105,벧후:61,요일:105,요이:13,요삼:14,유:25,계:404,
};
...
function GoalModal({ onClose, onSave }) {
  ...
  const toggleGroup = (books, allChecked) =>
    setSelected(prev => {
      const next = new Set(prev);
      allChecked ? books.forEach(a => next.delete(a)) : books.forEach(a => next.add(a));
      return next;
    });
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-7 py-4 border-b border-bluegrey-2 shrink-0">
          ...
          <button onClick={onClose} className="text-grey-5 hover:text-grey-9 text-xl leading-none">✕</button>
        </div>
        ...
        <div className="flex items-center gap-3 px-7 py-4 border-t border-bluegrey-2 shrink-0">
          <span className="text-body-5 text-grey-5 flex-1">{selected.size > 0 ? `${selected.size}권 선택됨` : ""}</span>
          ...
          <button onClick={() => { onSave(totalVerses, selected.size); onClose(); }}
...
function GoalBanner({ totalPct, monthGoalPct, hasGoal, onGoalSaved }) {
  ...
  const pct = goalVerse > 0 ? Math.min(100, Math.round((achievedVerse / goalVerse) * 100)) : monthGoalPct;
      ...
      <div className="mb-5 grid grid-cols-2 gap-4">
        ...
        <div className="border border-bluegrey-2 rounded-2xl px-5 py-4">
          ...
          <div className="h-1.5 bg-grey-3 rounded-full overflow-hidden">
            <div className="h-full bg-blue-7 rounded-full transition-all duration-500" style={{ width: `${totalPct}%` }} />
          </div>
        </div>
        ...
        <div className="border border-bluegrey-2 rounded-2xl px-5 py-4">
          ...
          {goalSet ? (
            <>
              ...
              <div className="h-1.5 bg-grey-3 rounded-full overflow-hidden">
                <div className="h-full bg-blue-5 rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
              </div>
              <p className="text-body-5 text-grey-5 mt-2">{achievedVerse.toLocaleString()} / {goalVerse.toLocaleString()}절</p>
            </>
          ) : (
    ...
    <div className="border border-bluegrey-2 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-body-5 text-grey-5">{yr}년 {mo + 1}월</p>
        </div>
        <div className="flex items-center gap-1">
          {!isThisMonth && (
            <button onClick={() => setCalDate(new Date())} className="px-2 py-0.5 text-caption text-blue-7 border border-blue-4 rounded-full hover:bg-blue-1 transition-colors mr-1">
              오늘
            </button>
          )}
      ...
      <div className="grid grid-cols-7 text-center mb-1">
        {["월","화","수","목","금","토","일"].map(d => (
          <span key={d} className="text-caption text-grey-5 font-medium pb-1">{d}</span>
        ))}
      </div>
            ...
            {[...Array(7)].map((_, di) => {
              ...
              return (
                <div key={di} className="flex items-center justify-center h-12 relative">
                  ...
                  {/* 날짜 원 */}
                  <span className={`relative z-10 w-8 h-8 flex items-center justify-center text-body-4 font-medium rounded-full transition-colors ${
                    read
...
function BibleProgress({ bookProgress, bookTab, setBookTab, navigate, mode }) {
  ...
  const handleTab = (key) => { setBookTab(key); setStartIdx(0); };
  // 뷰 모드 바뀌면 인덱스 초기화
  const handleViewMode = (v) => { setViewMode(v); setStartIdx(0); };
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => setStartIdx(i => Math.max(0, i - PAGE))} disabled={!canUp}
          ...
          <button onClick={() => setStartIdx(i => Math.min(Math.max(0, listBooks.length - PAGE), i + PAGE))} disabled={!canDown}
          ...
          <div className="flex border border-bluegrey-2 rounded-lg overflow-hidden">
            ...
            <button onClick={() => handleViewMode("grid")}
              ...
              title="바둑판 형태 (20권)">
              <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 3h8v8H3V3zm0 10h8v8H3v-8zm10-10h8v8h-8V3zm0 10h8v8h-8v-8z"/></svg>
            </button>
          </div>
        </div>
      </div>
              ...
              <button key={abbr}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-body-4 font-medium text-grey-9 group-hover:text-primary transition-colors">{BOOK_MAP[abbr]}</span>
                    <span className="text-body-5 text-grey-5">{pct}%</span>
                  </div>
                  <div className="h-1.5 bg-grey-2 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-5 rounded-full transition-all" style={{ width: `${pct}%` }} />
                  </div>
                </div>
                {pct === 100 && (
                  <svg className="w-4 h-4 text-blue-7 shrink-0" fill="currentColor" viewBox="0 0 24 24">
