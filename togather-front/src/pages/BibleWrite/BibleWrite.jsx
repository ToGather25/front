            ...
            const fontSizeClass =
              fontSize === "large"
                ...
                : fontSize === "small"
                  ? "text-body-2"
                  : "text-body-1";
            const chapterDone = completedVerses.filter(
              (v) => v.bookAbbr === selectedBook && v.chapter === selectedChapter,
            ).length;
            ...
            return (
              ...
              `}</style>
                ...
                <div className="shrink-0 px-10 pt-4 pb-2 flex items-center gap-3">
                  ...
                  <span className="text-caption text-grey-5 shrink-0">
                    {chapterDone} / {verses.length}절
                  </span>
                </div>
                  ...
                  <div className="flex flex-col gap-4 px-16">
                    {prevVerses.map((v, i) => {
                      ...
                      return (
                        ...
                        >
                          <svg
                            className="w-3.5 h-3.5 text-blue-4 shrink-0 mt-1"
                            fill="currentColor"
                            viewBox="0 0 24 24"
                          >
                            <path
                          ...
                          <p className="text-body-4 text-grey-6 leading-relaxed line-clamp-2">
                            <span className="text-grey-4 text-caption mr-1">{v.verse}</span>
                            {v.text}
                          </p>
                        </div>
                      );
                ...
                >
                  <div className="flex items-center gap-2 mb-5">
                    <span className="text-caption font-semibold text-primary tracking-wide">
                      {BOOK_MAP[selectedBook]} {selectedChapter}
                      {BOOK_MAP[selectedBook] === "시편" ? "편" : "장"} {selectedVerse}절
                    </span>
                    {isDone && (
                      <>
                  ...
                  <div className="relative">
                    <div
                      className={`${fontSizeClass} whitespace-pre-wrap tracking-wide select-none leading-relaxed`}
                    >
                      {targetText.split("").map((char, i) => {
                        if (i < typed.length) {
                          return (
                            <span
                              key={i}
                              className={typed[i] === char ? "text-grey-11" : "text-red-400"}
                            >
                              {typed[i]}
                            </span>
                          );
                        ...
                        return (
                          <span key={i} className={isDone ? "text-grey-11" : "text-grey-3"}>
                            {char}
                          </span>
                        );
                      })}
                  ...
                  <div className="flex flex-col gap-4 px-16">
                    {nextVerses.map((v, i) => {
                      ...
                      return (
                        <p
                          key={v.verse}
                          className="text-body-4 text-grey-5 leading-relaxed line-clamp-2 select-none"
                          style={{ opacity }}
                        >
                          <span className="text-grey-3 text-caption mr-1.5">{v.verse}</span>
                          {v.text}
                        </p>
                      );
                    })}
