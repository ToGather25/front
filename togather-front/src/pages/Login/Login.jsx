    ...
    <div className="max-w-md mx-auto mt-20 p-8 bg-white rounded-2xl shadow-xl border border-gray-100">
      ...
      <form action={formAction} className="flex flex-col gap-y-5">
        ...
        <button 
          type="submit" 
          disabled={isPending}
          className="w-full py-3 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 disabled:bg-blue-300 transition-colors"
        >
          {isPending ? "확인 중..." : "로그인"}
        </button>

        {state?.error && <p className="text-center text-red-500 text-sm font-medium">{state.error}</p>}
        {state?.message && <p className="text-center text-green-500 text-sm font-medium">{state.message}</p>}
      </form>
    </div>
  );
}