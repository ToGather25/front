...
function FloorGuide() {
  ...
  return (
    <div className="max-w-5xl mx-auto border border-bluegrey-2 rounded-2xl overflow-hidden">
      {church.floorGuide.map(({ floor, rooms }, i) => (
        <div
          key={floor}
          className={`flex items-stretch ${i < church.floorGuide.length - 1 ? "border-b border-grey-2" : ""}`}
        >
          <div className="w-32 shrink-0 bg-blue-1 flex items-center justify-center py-6">
            <span className="text-headline-4 font-bold text-primary">{floor}</span>
          </div>
          <div className="flex-1 flex items-center px-8 py-6 text-body-3 text-grey-8">
            {rooms}
          </div>
        </div>
      ))}
