const STATS = [
  { label: "전체 교인 수", value: "1,248명", sub: "지난달 대비 +12명", color: "#3b5280", icon: (
    <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
      <path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/>
      <path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/>
    </svg>
  )},
  { label: "이번 주 출석", value: "864명", sub: "출석률 69.2%", color: "#2563eb", icon: (
    <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
      <polyline points="9,11 12,14 22,4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/>
    </svg>
  )},
  { label: "신규 등록", value: "23명", sub: "이번 달 누계", color: "#059669", icon: (
    <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
      <path d="M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/>
      <line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/>
    </svg>
  )},
  { label: "온라인 조회", value: "3,521회", sub: "이번 주 홈페이지", color: "#d97706", icon: (
    <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-5 mb-8">
        {STATS.map(s => (
          <div key={s.label} className="bg-white rounded-2xl p-6 border border-grey-2 flex items-start gap-4">
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: "1fr 340px" }}>
        ...
        <div className="bg-white rounded-2xl border border-grey-2 p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex gap-1">
              {["1주", "2주", "3주", "4주"].map(w => (
                <button key={w} className="px-3 py-1 rounded-full text-body-5 text-grey-6 hover:bg-grey-2 transition-colors">{w}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-end gap-4 h-44">
            {[65, 82, 70, 78, 90, 74, 68].map((h, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                ...
                />
                <span className="text-[10px] text-grey-5">{["일", "월", "화", "수", "목", "금", "토"][i]}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-grey-2 p-6">
          ...
          <ul className="flex flex-col gap-2.5">
            {SCHEDULE.map((s, i) => (
              <li key={i} className="flex items-start gap-3 py-2.5 border-b border-grey-2 last:border-0">
      ...
      <div className="bg-white rounded-2xl border border-grey-2 p-6 mt-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <a href="/admin/notices" className="text-body-5 text-primary hover:underline">전체 보기</a>
        </div>
        <div className="divide-y divide-grey-2">
          {NOTICES.map((n, i) => (
            <div key={i} className="flex items-center gap-4 py-3">
              <span className="text-body-5 font-bold px-2 py-0.5 rounded text-primary bg-blue-1 shrink-0">{n.type}</span>
