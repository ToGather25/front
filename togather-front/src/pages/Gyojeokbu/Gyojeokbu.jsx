...
function AttendBar({ ytd }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-body-5 text-grey-6 font-mono shrink-0">{a}/{b}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[22px] font-bold text-grey-12 tracking-[-0.5px] leading-tight">
              {m.name}
              {m.nameRoman && <small className="ml-2 text-caption font-normal text-grey-5">{m.nameRoman}</small>}
            </div>
      ...
      <div className="flex-1 overflow-y-auto">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {(f.phone || target?.phone) && (
                        <div className="text-body-5 text-grey-6 mt-0.5 font-mono">{f.phone || target?.phone}</div>
                      )}
                    </div>
                    {target && (
            ...
            <div className="flex flex-col gap-2">
              {m.history.map((h, i) => (
                <div key={i} className="flex items-start gap-3">
                  <span className="text-body-5 font-bold text-blue-6 font-mono min-w-[36px] pt-0.5">{h.y}</span>
        ...
        <Section title="목회 메모">
          {m.notes
            ? <p className="text-body-3 text-grey-9 leading-[1.6] bg-bluegrey-1 rounded-xl p-4 m-0">{m.notes}</p>
...
function Section({ title, children }) {
  return (
    <div className="px-6 py-5 border-b border-bluegrey-2">
      <h4 className="text-caption font-bold text-grey-7 uppercase tracking-[0.06em] mb-3 m-0">{title}</h4>
      {children}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3.5">
        <span className="inline-flex items-center px-2 py-0.5 rounded bg-bluegrey-1 text-body-5 font-semibold text-grey-7">{m.region}</span>
      ...
      <div className="max-w-[1576px] mx-auto px-4 py-8 md:px-8 md:py-14">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3 mb-6 md:mb-8">
          ...
          <div className="flex gap-2.5">
            {[
              ...
              <button key={label} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-white border border-bluegrey-2 text-body-3 font-semibold text-grey-8 hover:border-blue-5 hover:text-blue-8 transition-colors">
                {icon}{label}
              </button>
            ))}
            ...
            <div key={s.label} className="bg-white rounded-2xl border border-bluegrey-2 p-6 flex items-start justify-between">
              <div>
                ...
                <div className="text-headline-3 font-bold text-grey-12 leading-none">
                  {s.val}<small className="text-body-2 font-normal ml-1 text-grey-6">{s.unit}</small>
                </div>
        ...
        <div className="bg-white rounded-2xl border border-bluegrey-2 px-6 py-4 flex items-center gap-4 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-3 h-11 px-4 bg-bluegrey-1 rounded-full border border-transparent focus-within:border-blue-6 focus-within:bg-white transition-all w-full md:w-[360px]">
            ...
            />
            {q && (
              <button onClick={() => setQ("")} className="w-5 h-5 rounded-full bg-grey-4 text-white text-body-5 flex items-center justify-center">×</button>
            )}
          </div>
        ...
        >
          ...
          {matches.length === 0 ? (
            <div className="py-24 text-center">
              ...
              <div className="text-sub-tit-3 font-bold text-grey-12 tracking-[-0.5px] mb-2">검색 결과가 없습니다</div>
              <div className="text-body-3 text-grey-5">이름 또는 휴대폰 번호 뒷자리를 다시 확인해 주세요.</div>
            </div>
          ) : (
