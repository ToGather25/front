...
function Avatar({ name, tone = 1, size = "sm" }) {
  ...
  const dim = size === "lg" ? "w-16 h-16 text-sub-tit-2" : "w-9 h-9 text-body-3";
  return (
    <div
...
function RoleChip({ role }) {
  ...
  return (
    <span className={`inline-flex px-2.5 py-1 rounded-md text-body-5 font-semibold ${cls}`}>
      {role}
    </span>
  );
}
...
function AttendBar({ ytd }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-body-5 text-grey-6 font-mono shrink-0">
        {a}/{b}
      </span>
    </div>
  );
    ...
    <div className="flex flex-col">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[22px] font-bold text-grey-12 tracking-[-0.5px] leading-tight">
              {m.name}
              {m.nameRoman && (
                <small className="ml-2 text-caption font-normal text-grey-5">{m.nameRoman}</small>
              )}
            </div>
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              <RoleChip role={m.role} />
              <span className="text-caption text-grey-6">{m.region}</span>
              {m.regionLeader && m.regionLeader !== "-" && (
                <span className="text-caption text-grey-5">· {m.regionLeader}</span>
              )}
            </div>
            <div className="flex gap-2 mt-3 flex-wrap">
              <a
                href={`tel:${m.phone}`}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-bluegrey-1 text-caption text-grey-8 hover:bg-blue-1 hover:text-blue-8 transition-colors"
              >
                <svg
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <Section title="교회 정보">
          ...
          <InfoRow
            label="구역"
            value={`${m.region}${m.regionLeader && m.regionLeader !== "-" ? ` · ${m.regionLeader}` : ""}`}
          />
        ...
        <Section title="출석 현황">
          <div className="grid grid-cols-3 gap-3">
            {[
              {
                label: "최근 출석일",
                val: m.lastAttend.split(" ")[0],
                sub: m.lastAttend.split(" ")[1] || "",
              },
              ...
              <div key={cell.label} className="bg-bluegrey-1 rounded-xl p-3 text-center">
                ...
                <div className="text-body-2 font-bold text-grey-12">{cell.val}</div>
              ...
              {m.family.map((f, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-semibold text-grey-5 bg-bluegrey-1 px-1.5 py-0.5 rounded">
                          {f.rel}
                        </span>
                        <span className="text-body-3 font-semibold text-grey-11">{f.name}</span>
                      </div>
                      {(f.phone || target?.phone) && (
                        <div className="text-body-5 text-grey-6 mt-0.5 font-mono">
                          {f.phone || target?.phone}
                        </div>
                      )}
                    </div>
            ...
            <div className="flex flex-col gap-2">
              {m.history.map((h, i) => (
                <div key={i} className="flex items-start gap-3">
                  <span className="text-body-5 font-bold text-blue-6 font-mono min-w-[36px] pt-0.5">
                    {h.y}
                  </span>
                  <div className="flex-1 h-px bg-bluegrey-2 self-center mx-2" />
                  <span className="text-body-3 text-grey-9">{h.t}</span>
                </div>
              ))}
            </div>
          </Section>
...
function Section({ title, children }) {
  return (
    <div className="px-6 py-5 border-b border-bluegrey-2">
      <h4 className="text-caption font-bold text-grey-7 uppercase tracking-[0.06em] mb-3 m-0">
        {title}
      </h4>
      {children}
    </div>
...
function InfoRow({ label, value, mono }) {
  return (
    <div className="flex items-start gap-2 py-1.5">
      <span className="text-caption text-grey-5 w-20 shrink-0 pt-0.5">{label}</span>
      <span className={`text-body-3 text-grey-11 flex-1 ${mono ? "font-mono" : ""}`}>{value}</span>
    </div>
  );
}
    ...
    >
      <td className="px-6 py-3.5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-body-3 font-semibold text-grey-12">{highlight(m.name)}</div>
            {m.nameRoman && <div className="text-body-5 text-grey-5">{m.nameRoman}</div>}
          </div>
        </div>
      </td>
      ...
      <td className="px-4 py-3.5">
        <span className="inline-flex items-center px-2 py-0.5 rounded bg-bluegrey-1 text-body-5 font-semibold text-grey-7">
          {m.region}
        </span>
        <div className="text-body-5 text-grey-5 mt-1">{m.smallGroup}</div>
      </td>
      <td className="px-4 py-3.5 font-mono text-caption text-grey-8">
        {m.birth}
        <div className="text-[11px] text-grey-5 mt-0.5">만 {m.age}세</div>
      </td>
      <td className="px-4 py-3.5 font-mono text-caption text-grey-8">{highlight(m.phone)}</td>
      <td className="px-4 py-3.5 font-mono text-caption text-grey-8">
        {lastDate}
      ...
      <td className="px-4 py-3.5 text-right">
        <svg
          className="w-4 h-4 text-grey-4 inline"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          viewBox="0 0 24 24"
      ...
      <div className="max-w-[1576px] mx-auto px-4 py-8 md:px-8 md:py-14">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3 mb-6 md:mb-8">
          <div>
            <div className="flex items-center gap-2 text-body-3 text-grey-6 mb-3">
              <Link to="/" className="hover:text-blue-6 transition-colors">
                홈
              </Link>
              <span className="text-grey-4">/</span>
              <span className="text-blue-6 font-semibold">교적부</span>
            </div>
            <h1 className="text-headline-2 font-bold tracking-[-1.5px] text-grey-12 m-0">교적부</h1>
          </div>
          <div className="flex gap-2.5">
            {[
              ...
              <button
                key={label}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-white border border-bluegrey-2 text-body-3 font-semibold text-grey-8 hover:border-blue-5 hover:text-blue-8 transition-colors"
              >
                {icon}
                {label}
              </button>
            ))}
            <button className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-primary text-white text-body-3 font-semibold hover:bg-blue-8 transition-colors">
              <svg
                className="w-4 h-4"
                fill="none"
                stroke="currentColor"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-5 md:mb-7">
          {[
            ...
            <div
              key={s.label}
              className="bg-white rounded-2xl border border-bluegrey-2 p-6 flex items-start justify-between"
            >
              <div>
                <div className="text-caption text-grey-6 mb-2">{s.label}</div>
                <div className="text-headline-3 font-bold text-grey-12 leading-none">
                  {s.val}
                  <small className="text-body-2 font-normal ml-1 text-grey-6">{s.unit}</small>
                </div>
                <div className={`text-body-5 mt-2 ${s.color}`}>{s.sub}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-bluegrey-2 px-6 py-4 flex items-center gap-4 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-3 h-11 px-4 bg-bluegrey-1 rounded-full border border-transparent focus-within:border-blue-6 focus-within:bg-white transition-all w-full md:w-[360px]">
            ...
            {q && (
              <button
                onClick={() => setQ("")}
                className="w-5 h-5 rounded-full bg-grey-4 text-white text-body-5 flex items-center justify-center"
              >
                ×
              </button>
            )}
            ...
            {FILTER_DEFS.map((f) => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-caption font-semibold transition-all ${
                    filter === f.k
                ...
                >
                  {f.k}
                  <span
                    className={`text-[11px] px-1.5 py-0.5 rounded-full ${filter === f.k ? "bg-white/20 text-white" : "bg-bluegrey-2 text-grey-6"}`}
                  >
                    {n}
                  </span>
                </button>
          ...
          {/* Count */}
          <div className="ml-auto shrink-0 text-body-3 text-grey-6">
        ...
        {member && (
          <>
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-bluegrey-2 bg-white shrink-0">
              ...
              <span className="text-caption text-grey-5 font-mono">
